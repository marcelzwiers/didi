function FNames = spm_dicom_convert_mz(hdr,opts,root_dir,format) % Modified by Marcel
...
if (strcmp(opts,'all') || strcmp(opts,'mosaic')) && ~isempty(mosaic),
    FNames = convert_mosaic(mosaic,root_dir,format); % Modified by Marcel
    % convert_mosaic(mosaic,root_dir,format);
end;
if (strcmp(opts,'all') || strcmp(opts,'standard')) && ~isempty(standard),
    FNames = convert_standard(standard,root_dir,format); % Modified by Marcel
...
FNames = strvcat(FNames);           % Added by Marcel
if nargout==0, clear('FNames'), end % Added by Marcel
return;
%_______________________________________________________________________

%_______________________________________________________________________
function FNames = convert_mosaic(hdr,root_dir,format) % Modified by Marcel
...
for i=1:length(hdr),
    ...
    FNames{i} = fname;  % Added by Marcel
    ...
    catch % Added by Marcel
        tim = datevec(hdr{i}.AcquisitionDate);
    end
...
%_______________________________________________________________________
function FNames = convert_standard(hdr,root_dir,format) % Modified by Marcel
%function convert_standard(hdr,root_dir,format)
hdr = sort_into_volumes(hdr);
for i=1:length(hdr),
    % write_volume(hdr{i},root_dir,format);
    FNames{i} = write_volume(hdr{i},root_dir,format); % Modified by Marcel
end;
return;
%_______________________________________________________________________
...
% dcm = vol;
% save('dicom_headers.mat','dcm'); % Modified by Marcel
...
%_______________________________________________________________________
function fname = write_volume(hdr,root_dir,format) % Modified by Marcel
...
if strcmp(root_dir, 'flat')
    ...
    if checkfields(hdr,'ProtocolName','PatientsName') % if-elseif added by Marcel (replaced prefix and PatientID)
        if checkfields(hdr,'EchoNumbers')
           fname = sprintf('%s-%s-%.4d-%.5d-%.6d-%.2d.%s', hdr.PatientsName, hdr.ProtocolName,...
               hdr.SeriesNumber, hdr.AcquisitionNumber, hdr.InstanceNumber,...
               hdr.EchoNumbers, format);
        else
           fname = sprintf('%s-%s-%.4d-%.5d-%.6d.%s', hdr.PatientsName, hdr.ProtocolName,...
               hdr.SeriesNumber, hdr.AcquisitionNumber, ...
               hdr.InstanceNumber, format);
        end
    elseif checkfields(hdr,'SeriesNumber','AcquisitionNumber')
        if checkfields(hdr,'EchoNumbers')
    ...
    fname = strrep(fname, ' ', '_'); % Modified by Marcel
    fname = strrep(fname, '/', '_'); % Modified by Marcel
    fname = strrep(fname, '\', '_'); % Modified by Marcel
