...
void medianfilter(float *A, float *B, size_t nx, size_t ny, char *w,
        ...
        for (size_t i = 0; i < nx; i++)
        {
            ...
            for (size_t wj = wj1; wj <= wj2; wj++)
                for (size_t wi = wi1; wi <= wi2; wi++)
                    if (w[wj * nwx + wi] > 0)
                        buf[nbuf++] = A[(j - hnwy + wj) * nx + i - hnwx + wi];
            
            int mi = nbuf / 2;
            std::nth_element(buf, buf + mi, buf + nbuf);
            *B++ = buf[mi];
        }
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for (size_t k = 0; k < imgz; k++)
    {
        ...
        medianfilter(img + offs, newimg + offs, imgx, imgy, w, wx, wy);
    }
}
